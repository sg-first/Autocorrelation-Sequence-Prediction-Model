...
city = "D"#改
slight_list_A=[28,59,69,73,85,91,95,100,115,116]
slight_list_B=[2,6,12,13,14,15,18,19,21,22,23,25,27,29]
slight_list_C=[3,7,15,16,21,25,35,39,68,78,79,92,115,119,126,128,132]
slight_list_D=[4,5,6,7,10,11,15,18,21,22,24,26,30,31,32,34,36,37,41,42,45,46,47,50,51,53,54,56,57,58,59,62,63,64,66,67,68,69,72,73,74]
slight_list_E=[0,3,5,9,16,18,19,20,22,23,24,27,29,30,32,33]
def sort(df):
...
cant_list=list()
for i in slight_list_D[slight_list_D.index(10):]:#改
    callback_list = [
    ...
    plt.legend(loc="best")
    plt.savefig("F:\emsenble_slight_{}\{}城{}区{}拟合.png".format(city, city, str(i), str(j)))

    if ( slight_list_D.index(i)== 0):#改
        result_final = pd.DataFrame(result, index=[i] * result.shape[0], columns=["天数", "感染人数"])
        result_final.to_csv("predict_bei_slight_{}.csv".format(city), columns=["天数", "感染人数"])
    else:
        result_final = pd.read_csv("predict_bei_slight_{}.csv".format(city), names=["天数", "感染人数"])
